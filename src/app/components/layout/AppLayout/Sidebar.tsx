...
const menuItems: MenuItem[] = [
    ...
    { name: 'Miu Kute', path: '/child', icon: GiCutDiamond, iconSize: 26, url: "/assets/images/blog.png", width: 30, height: 30 },
    ...
    { name: 'Image', path: '/image', icon: CiImageOn, iconSize: 24, url: "/assets/images/image.png", width: 30, height: 30 },
...
const ItemMenu = ({ item, isShowSidebar }: { item: typeof menuItems[0]; isShowSidebar: boolean }) => {
    ...
    return (
        ...
        >
            ...
            <span className={`${isShowSidebar ? "w-0 opacity-0" : "flex-1"} transition-width transition-opacity duration-500 ease-linear overflow-hidden font-bold'`}>{item.name}</span>
        </Link>
    )
}
...
export default function Sidebar({ isShowSidebar }: SidebarProps) {
    ...
    return (
        <div className={`${isShowSidebar ? "w-[60px]" : "w-3xs"} "wrapp-sidebar py-3 bg-darkcharcoal text-white fixed h-screen transition-all duration-500 ease-linear"`}>
            <div className={`${isShowSidebar ? "" : ""} items-center logo flex  gap-2 mb-4 px-3 transition-all duration-500 ease-linear "`}>
                <Image src="/assets/images/icon-logo.png" alt="File icon" width={40} height={40} className='min-w-[40px] w-[40px] h-[40px]' />
                <h1 className={`${isShowSidebar ? "w-[0px] opacity-0" : "w-auto opacity-100"} text-3xl font-semibold transition-all duration-500 ease-linear overflow-hidden bg-gradient-to-r
